...
def get_product_by_name(
    product_name: str, _auth: dict = Depends(has_any_role(["ROLE_USER", "ROLE_ADMIN"]))
):
...
def update_product(
    ...
    return product_service.update_product(
        product_id, file=file, product_name=product_name, product_price=product_price
    )


@router.delete("/{product_id}")
def delete_product(
    product_id: int, _auth: dict = Depends(has_any_role(["ROLE_ADMIN"]))
):
